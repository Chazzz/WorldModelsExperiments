...
class Model:
  ...
  def __init__(self, load_model=True):
    ...
    if EXP_MODE == MODE_Z_HIDDEN: # one hidden layer
      ...
      self.weight_output = np.random.randn(self.hidden_size, self.num_actions)
      self.bias_output = np.random.randn(self.num_actions)
      self.param_count = ((self.input_size+1)*self.hidden_size) + ((self.hidden_size+1)*self.num_actions)
    else:
  ...
  def set_model_params(self, model_params):
    if EXP_MODE == MODE_Z_HIDDEN: # one hidden layer
      ...
      self.weight_output = params_2[self.num_actions:].reshape(self.hidden_size, self.num_actions)
    else:
      self.bias = np.array(model_params[:self.num_actions])
      self.weight = np.array(model_params[self.num_actions:]).reshape(self.input_size, self.num_actions)
