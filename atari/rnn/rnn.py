...
def default_hps():
  return HyperParams(num_steps=2000, # train model for 2000 steps.
                     ...
                     num_actions=6,    # width of our action space (from env.action_space.n)
                     encoded_image_width=32,    # width of our data is 32
...
class MDNRNN():
  ...
  def build_model(self, hps):
    ...
    INWIDTH = hps.encoded_image_width + hps.num_actions # 35 channels
    OUTWIDTH = hps.encoded_image_width # 32 channels
...
def rnn_next_state(rnn, z, a, prev_state):
  # a is a one hot array where selected action is the hot value
  input_x = np.concatenate((z.reshape((1, 1, 32)), a.reshape((1, 1, len(a)))), axis = 2)
