...
def create_dataset_lazy(filelist, N=10000*1000, M=1): #N=6000*1000
  data = []
  idx = 0
  for i in range(N):
    filename = filelist[i]
...
def create_dupes_dict(filename):
  dupes = {}
  with open(filename) as f:
    previous_line = "\n"
    for line in f:
      if line != "\n" and previous_line != "\n":
        dupes[line.split("/")[-1][:-1]] = True # Remove trailing \n
      previous_line = line
  return dupes
...
if lazy:
  ...
  dupes = create_dupes_dict("dupes.txt") # "dupes.txt" generated by "fdupes record_images_uncompressed > dupes.txt"
  print(dataset[0])
  if remove_duplicates:
    dataset = [d for d in dataset if d[0] not in dupes]
    print("total non-dupe images:", len(dataset))
else:
  filelist = os.listdir(DATA_DIR)
  filelist.sort()
  filelist = filelist[0:12500]
