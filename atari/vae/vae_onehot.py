...
class ConvVAEOH(object):
  ...
  def _build_graph(self):
    ...
    with self.g.as_default():
      ...
      self.x = tf.placeholder(tf.float32, shape=[None, 8, 8, 1])

      # hack = tf.reshape(self.x, [-1, 64])
      # hack = tf.layers.dense(hack, 64)
      # self.z = tf.layers.dense(hack, 32)
      # hack = tf.layers.dense(self.z, 64)
      # hack = tf.layers.dense(hack, 64)
      # self.y = tf.reshape(hack, [-1, 8, 8, 1])
